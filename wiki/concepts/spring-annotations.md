...
> 상세 비교(vs `@AllArgsConstructor`)는 [[lombok]] 참고.
...
| 어노테이션 | 한 줄 의미 | 깊이 보기 |
|---|---|---|
| `@EnableWebSecurity` | Spring Security 웹 보안 기능 활성화 | [[spring-security]] |
| `@Valid` | Controller 파라미터 DTO를 Bean Validation 규칙으로 검증 트리거 | [[bean-validation]] |
| `@NotNull` | null 금지 (모든 타입) | [[bean-validation]] |
| `@NotBlank` | null + 빈 문자열 + 공백 금지 (String 전용) | [[bean-validation]] |
| `@Email` | 이메일 형식 검증 | [[bean-validation]] |
| `@Size(min, max)` | 길이/크기 범위 제한 | [[bean-validation]] |
| `@Pattern(regexp)` | 정규식 패턴 일치 | [[bean-validation]] |

> `@Valid`가 없으면 Controller 파라미터에 붙인 `@NotBlank` 등이 **동작하지 않는다**.
> 트리거 역할이므로 반드시 메서드 파라미터에 함께 붙여야 한다.

## 8. JPA Auditing

| 어노테이션 | 한 줄 의미 | 깊이 보기 |
|---|---|---|
| `@EnableJpaAuditing` | JPA Auditing 기능 전체 활성화 (Application 클래스) | [[jpa-auditing]] |
| `@MappedSuperclass` | 테이블 없이 상속 전용 슈퍼클래스 선언 (BaseEntity 패턴) | [[jpa-auditing]] |
| `@EntityListeners(...)` | 엔티티 생명주기 이벤트 리스너 연결 | [[jpa-auditing]] |
| `@CreatedDate` | insert 시 날짜/시각 자동 주입 | [[jpa-auditing]] |
| `@LastModifiedDate` | insert + update 시 날짜/시각 자동 갱신 | [[jpa-auditing]] |
...
- [[jpa-entity-mapping]] · [[jpa-association]] · [[entity-manager]] · [[jpa-auditing]] — JPA 매핑 어노테이션
- [[spring-security]] · [[bean-validation]] — 보안·검증 어노테이션
- [[dispatcher-servlet]] · [[restful-api]] · [[http-status-codes]] — 웹 요청·응답 어노테이션
- 출처: [[spring-transaction-proxy-dialogue]] · [[zeroverse-spring-practice]]
