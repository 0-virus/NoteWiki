...
## Jakarta EE 마이그레이션 주의 (Spring Boot 3.x)

`javax.servlet.*` 패키지가 **`jakarta.servlet.*`** 으로 이름이 바뀌었다.
Spring Boot 2.x까지는 `import javax.servlet.http.HttpServlet`, Boot 3.x부터는 `import jakarta.servlet.http.HttpServlet`.
`pom.xml`/`build.gradle`의 의존성도 `javax.servlet:javax.servlet-api` → Jakarta EE 관련 artifact로 바뀌관.
`@WebServlet`, `HttpServletRequest`, `HttpServletResponse` 등 모두 해당. ([[spring-framework]] 버전 역사 참고)
